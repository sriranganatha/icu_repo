...
/// <summary>
/// Publishes domain events from InpatientService to its designated Kafka topic.
/// Uses transactional outbox pattern in production; direct produce in dev.
/// </summary>
public sealed class InpatientServiceEventProducer : IAsyncDisposable
{
    private readonly IProducer<string, string> _producer;
    private readonly ILogger<InpatientServiceEventProducer> _logger;
    private readonly string _topic = KafkaTopics.For("inpatient");

    public InpatientServiceEventProducer(
        IProducer<string, string> producer,
        ILogger<InpatientServiceEventProducer> logger)
    {
        _producer = producer;
        _logger = logger;
    }
