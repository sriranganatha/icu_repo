...
{
    private readonly Mock<IEncounterRepository> _repoMock = new();
    ...
    public EncounterServiceTests()
    {
        ...
        _sut = new EncounterService(
            _repoMock.Object,
            _eventsMock.Object,
            Mock.Of<ILogger<EncounterService>>());
    }

    private static Encounter CreateTestEntity() => new()
    {
    ...
    {
        var entities = new List<Encounter> { CreateTestEntity(), CreateTestEntity() };
    ...
    {
        _repoMock.Setup(r => r.CreateAsync(It.IsAny<Encounter>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync((Encounter e, CancellationToken _) => e);
        ...
        _repoMock.Verify(r => r.CreateAsync(It.IsAny<Encounter>(), It.IsAny<CancellationToken>()), Times.Once);
    }
    ...
    public async Task Create_PublishesCreatedEvent()
    {
        ...
        _eventsMock.Verify(e => e.PublishAsync(
            It.Is<EncounterCreatedEvent>(evt => evt.TenantId == "tenant-1"),
    ...
    public async Task Update_ThrowsWhenNotFound()
    {
        ...
        var act = () => _sut.UpdateAsync(new UpdateEncounterRequest { Id = "missing" });
    ...
    public async Task Update_PublishesUpdatedEvent()
    {
        ...
        await _sut.UpdateAsync(new UpdateEncounterRequest { Id = entity.Id });

        _repoMock.Verify(r => r.UpdateAsync(It.IsAny<Encounter>(), It.IsAny<CancellationToken>()), Times.Once);
        _eventsMock.Verify(e => e.PublishAsync(
            It.Is<EncounterUpdatedEvent>(evt => evt.EntityId == entity.Id),
            It.IsAny<CancellationToken>()), Times.Once);
    }
    ...
    {
        typeof(Encounter).GetProperty("TenantId").Should().NotBeNull(
