...
{
    private readonly Mock<IAdmissionRepository> _repoMock = new();
    ...
    public AdmissionServiceTests()
    {
        ...
        _sut = new AdmissionService(
            _repoMock.Object,
            _eventsMock.Object,
            Mock.Of<ILogger<AdmissionService>>());
    }

    private static Admission CreateTestEntity() => new()
    {
    ...
    {
        var entities = new List<Admission> { CreateTestEntity(), CreateTestEntity() };
    ...
    {
        _repoMock.Setup(r => r.CreateAsync(It.IsAny<Admission>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync((Admission e, CancellationToken _) => e);
        ...
        _repoMock.Verify(r => r.CreateAsync(It.IsAny<Admission>(), It.IsAny<CancellationToken>()), Times.Once);
    }
    ...
    public async Task Create_PublishesCreatedEvent()
    {
        ...
        _eventsMock.Verify(e => e.PublishAsync(
            It.Is<AdmissionCreatedEvent>(evt => evt.TenantId == "tenant-1"),
    ...
    public async Task Update_ThrowsWhenNotFound()
    {
        ...
        var act = () => _sut.UpdateAsync(new UpdateAdmissionRequest { Id = "missing" });
    ...
    public async Task Update_PublishesUpdatedEvent()
    {
        ...
        await _sut.UpdateAsync(new UpdateAdmissionRequest { Id = entity.Id });

        _repoMock.Verify(r => r.UpdateAsync(It.IsAny<Admission>(), It.IsAny<CancellationToken>()), Times.Once);
        _eventsMock.Verify(e => e.PublishAsync(
            It.Is<AdmissionUpdatedEvent>(evt => evt.EntityId == entity.Id),
            It.IsAny<CancellationToken>()), Times.Once);
    }
    ...
    {
        typeof(Admission).GetProperty("TenantId").Should().NotBeNull(
