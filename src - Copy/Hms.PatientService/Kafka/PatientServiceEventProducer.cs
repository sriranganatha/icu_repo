...
/// <summary>
/// Publishes domain events from PatientService to its designated Kafka topic.
/// Uses transactional outbox pattern in production; direct produce in dev.
/// </summary>
public sealed class PatientServiceEventProducer : IAsyncDisposable
{
    private readonly IProducer<string, string> _producer;
    private readonly ILogger<PatientServiceEventProducer> _logger;
    private readonly string _topic = KafkaTopics.For("patient");

    public PatientServiceEventProducer(
        IProducer<string, string> producer,
        ILogger<PatientServiceEventProducer> logger)
    {
        _producer = producer;
        _logger = logger;
    }
