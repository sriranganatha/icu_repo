...
public class WorkflowAndProjectPipelineTests
{
    ...
    private static AgentOrchestrator CreateOrchestrator(List<IAgent> agents)
    {
        ...
        llm.SetupGet(x => x.IsAvailable).Returns(true);
        llm.SetupGet(x => x.ProviderName).Returns("test-llm");
        llm.Setup(x => x.GenerateAsync(It.IsAny<LlmPrompt>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(new LlmResponse { Success = true, Content = "ok", Model = "test-model" });
