...
namespace HmsAgents.Agents.CodeReasoning;
...
public sealed class CodeReasoningAgent : IAgent
{
            ...
            foreach (var entity in svcGroup)
            {
                ...
                if (entityOnly.Count > 0)
                {
                    findings.Add(new ReviewFinding
                    {
                        ...
                        FilePath = $"src/Hms.{svcGroup.Key}/DTOs/{entity.Name}Dto.cs",
        ...
        foreach (var (evt, publisher) in published)
        {
            if (!consumed.ContainsKey(evt))
            {
                findings.Add(new ReviewFinding
                {
                    ...
                    FilePath = $"src/Hms.{publisher}/Events/{evt}.cs",
        ...
        foreach (var svcName in consumingServices)
        {
            ...
            if (consumerArtifacts.Count == 0 && eventsConsumedByService.Count > 0)
            {
                findings.Add(new ReviewFinding
                {
                    ...
                    FilePath = $"src/Hms.{svcName}/Consumers/",
    ...
    private static List<ReviewFinding> CheckNamingConventions(AgentContext context)
    {
        ...
        // All namespaces should start with "Hms"
        foreach (var group in namespaceGroups)
        {
            if (!group.Key.StartsWith("Hms", StringComparison.OrdinalIgnoreCase))
            {
                findings.Add(new ReviewFinding
                {
                    Category = "Conventions",
                    Severity = ReviewSeverity.Warning,
                    Message = $"Namespace root '{group.Key}' doesn't follow project convention — expected 'Hms.*'. Affects {group.Count()} artifacts.",
                    FilePath = group.First().RelativePath,
                    Suggestion = "Change namespace root to 'Hms.{ServiceName}.{Layer}'."
                });
            }
        }
