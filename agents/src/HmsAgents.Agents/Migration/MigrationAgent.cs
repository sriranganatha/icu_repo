...
namespace HmsAgents.Agents.Migration;
...
public sealed class MigrationAgent : IAgent
{
    ...
    public async Task<AgentResult> ExecuteAsync(AgentContext context, CancellationToken ct = default)
    {
        ...
        try
        {
            foreach (var svc in ServiceNames)
            {
                ...
                var projDir = Path.Combine(context.OutputBasePath, "src", $"Hms.{svc}");
                ...
                var migrationPrompt = $"""
                    ...
                    Use .NET 8 / EF Core 8 syntax. Namespace: Hms.{svc}.Migrations
                ...
                artifacts.Add(new CodeArtifact
                {
                    ...
                    RelativePath = $"src/Hms.{svc}/Migrations/InitialCreate.cs",
                    FileName = "InitialCreate.cs",
                    Namespace = $"Hms.{svc}.Migrations",
                    ProducedBy = Type,
                ...
                var seedPrompt = $"""
                    ...
                    Use .NET 8 syntax. Namespace: Hms.{svc}.Data
                    """;
                ...
                {
                    Layer = ArtifactLayer.Database,
                    RelativePath = $"src/Hms.{svc}/Data/{svc}DataSeeder.cs",
                    FileName = $"{svc}DataSeeder.cs",
                    Namespace = $"Hms.{svc}.Data",
                    ProducedBy = Type,
                ...
                {
                    Layer = ArtifactLayer.Database,
                    RelativePath = $"src/Hms.{svc}/Migrations/rollback.sql",
    ...
    private static CodeArtifact GenerateMigrationRunnerConfig() => new()
    {
        ...
        Content = """
            ...
            param(
                [string]$ConnectionString = "Host=localhost;Port=5432;Database=hms;Username=hms;Password=hms_dev_pw",
                [switch]$Rollback
            )

            $services = @(
                "Hms.PatientService",
                "Hms.EncounterService",
                "Hms.InpatientService",
                "Hms.EmergencyService",
                "Hms.DiagnosticsService",
                "Hms.RevenueService",
                "Hms.AuditService",
                "Hms.AiService"
            )
