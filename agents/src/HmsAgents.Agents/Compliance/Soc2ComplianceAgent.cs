...
public sealed class Soc2ComplianceAgent : IAgent
{
    ...
    private async Task<CodeArtifact> GenerateChangeManagementGate(CancellationToken ct)
    {
        ...
        return new CodeArtifact
        {
            ...
            RelativePath = "Hms.SharedKernel/Compliance/Soc2/ChangeManagementGate.cs",
    ...
    private async Task<CodeArtifact> GenerateAccessReviewService(CancellationToken ct)
    {
        var response = await _llm.GenerateAsync(new LlmPrompt
        {
            ...
            UserPrompt = "Generate an AccessReviewService that: 1) Lists all user→role mappings, 2) Flags dormant accounts (>90 days), 3) Flags over-privileged users, 4) Generates quarterly access review report. Namespace: Hms.SharedKernel.Compliance.Soc2.",
            Temperature = 0.1, RequestingAgent = Name
        }, ct);
        ...
        {
            Layer = ArtifactLayer.Compliance,
            RelativePath = "Hms.SharedKernel/Compliance/Soc2/AccessReviewService.cs",
    ...
    private async Task<CodeArtifact> GenerateIncidentResponsePlan(CancellationToken ct)
    {
        var response = await _llm.GenerateAsync(new LlmPrompt
        {
            ...
            UserPrompt = "Generate an IncidentResponseService with: 1) ReportIncident, 2) ClassifySeverity (P1-P4), 3) EscalationPath per severity, 4) PostIncidentReview. Include IncidentRecord entity. Namespace: Hms.SharedKernel.Compliance.Soc2.",
            Temperature = 0.1, RequestingAgent = Name
        }, ct);
        ...
        {
            Layer = ArtifactLayer.Compliance,
            RelativePath = "Hms.SharedKernel/Compliance/Soc2/IncidentResponseService.cs",
    ...
    private async Task<CodeArtifact> GenerateBackupDrPolicy(CancellationToken ct)
    {
        var response = await _llm.GenerateAsync(new LlmPrompt
        {
            ...
            UserPrompt = "Generate a BackupDrPolicy class with: RPO/RTO targets per service tier, backup schedule configs, failover procedures, and recovery test schedule. Namespace: Hms.SharedKernel.Compliance.Soc2.",
            Temperature = 0.1, RequestingAgent = Name
        }, ct);
        ...
        {
            Layer = ArtifactLayer.Compliance,
            RelativePath = "Hms.SharedKernel/Compliance/Soc2/BackupDrPolicy.cs",
    ...
    {
        Layer = ArtifactLayer.Compliance,
        RelativePath = "Hms.SharedKernel/Compliance/Soc2/Soc2ControlMatrix.cs",
