...
public sealed class AgentPipelineDb : IDisposable
{
    ...
    private void InitializeSchema()
    {
        ...
        cmd.CommandText = """
            ...
            ALTER TABLE PipelineRuns ADD COLUMN IF NOT EXISTS ProjectId TEXT;
            ALTER TABLE Requirements ADD COLUMN IF NOT EXISTS ProjectId TEXT;
            ALTER TABLE BacklogItems ADD COLUMN IF NOT EXISTS ProjectId TEXT;
            ALTER TABLE Findings ADD COLUMN IF NOT EXISTS ProjectId TEXT;
            ALTER TABLE Artifacts ADD COLUMN IF NOT EXISTS ProjectId TEXT;
            ALTER TABLE TestDiagnostics ADD COLUMN IF NOT EXISTS ProjectId TEXT;

            CREATE INDEX IF NOT EXISTS IX_PipelineRuns_ProjectId ON PipelineRuns(ProjectId);
            CREATE INDEX IF NOT EXISTS IX_Requirements_ProjectId ON Requirements(ProjectId);
            CREATE INDEX IF NOT EXISTS IX_BacklogItems_ProjectId ON BacklogItems(ProjectId);
            CREATE INDEX IF NOT EXISTS IX_Findings_ProjectId ON Findings(ProjectId);
    ...
    public void StartRun(string runId, string? instructions, string? configJson, string? projectId = null)
    {
        ...
        using var cmd = conn.CreateCommand();
        cmd.CommandText = """
            INSERT INTO PipelineRuns (RunId, ProjectId, Status, StartedAt, Instructions, ConfigJson)
            VALUES (@runId, @projectId, 'Running', @now, @instructions, @configJson)
        """;
    ...
    public void SaveRequirements(string runId, IEnumerable<RequirementRow> items)
    {
        ...
        try
        {
            ...
            foreach (var item in items)
            {
                using var cmd = conn.CreateCommand();
                cmd.CommandText = """
                    INSERT INTO Requirements (Id, RunId, ProjectId, SourceFile, Section, HeadingLevel, Title, Description,
                        Module, Tags, AcceptanceCriteria, DependsOn, CreatedAt)
                    VALUES (@id, @runId, @projectId, @src, @section, @heading, @title, @desc,
    ...
    public void SaveFindings(string runId, IEnumerable<FindingRow> items)
    {
        ...
        try
        {
            ...
            foreach (var f in items)
            {
                using var cmd = conn.CreateCommand();
                cmd.CommandText = """
                    INSERT INTO Findings (Id, RunId, ProjectId, ArtifactId, FilePath, LineNumber, Severity, Category, Message, Suggestion, TracedRequirementId)
                    VALUES (@id, @runId, @projectId, @artId, @path, @line, @sev, @cat, @msg, @sug, @reqId)
                """;
                cmd.Parameters.AddWithValue("@id", f.Id);
                cmd.Parameters.AddWithValue("@runId", runId);
                cmd.Parameters.AddWithValue("@projectId", (object?)f.ProjectId ?? DBNull.Value);
    ...
    public void SaveArtifacts(string runId, IEnumerable<ArtifactRow> items)
    {
        ...
        try
        {
            ...
            foreach (var a in items)
            {
                ...
                cmd.Parameters.AddWithValue("@runId", runId);
                cmd.Parameters.AddWithValue("@projectId", (object?)a.ProjectId ?? DBNull.Value);
    ...
    public void SaveTestDiagnostics(string runId, IEnumerable<TestDiagRow> items)
    {
        ...
        try
        {
            ...
            foreach (var d in items)
            {
                using var cmd = conn.CreateCommand();
                cmd.CommandText = """
                    INSERT INTO TestDiagnostics (Id, RunId, ProjectId, TestName, AgentUnderTest, Outcome, Diagnostic, Remediation, Category, DurationMs, AttemptNumber, Timestamp)
                    VALUES (@id, @runId, @projectId, @name, @agent, @outcome, @diag, @rem, @cat, @dur, @attempt, @ts)
                """;
                cmd.Parameters.AddWithValue("@id", d.Id);
                cmd.Parameters.AddWithValue("@runId", runId);
                cmd.Parameters.AddWithValue("@projectId", (object?)d.ProjectId ?? DBNull.Value);
    ...
    public PipelineRunRow? GetLatestRun()
    {
        ...
        return new PipelineRunRow
        {
            ...
            ProjectId = r.IsDBNull(r.GetOrdinal("ProjectId")) ? null : r.GetString(r.GetOrdinal("ProjectId")),
