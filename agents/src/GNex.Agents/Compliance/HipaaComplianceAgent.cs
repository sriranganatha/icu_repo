...
public sealed class HipaaComplianceAgent : IAgent
{
    ...
    private async Task<CodeArtifact> GeneratePhiInventory(string domainSummary, string domainContext, CancellationToken ct)
    {
        ...
        return new CodeArtifact
        {
            ...
            RelativePath = "GNex.SharedKernel/Compliance/PhiInventory.cs",
    ...
    private async Task<CodeArtifact> GenerateAccessAuditService(string domainSummary, CancellationToken ct)
    {
        var response = await _llm.GenerateAsync(new LlmPrompt
        {
            ...
            UserPrompt = "Generate a PhiAccessAuditService class with methods: LogAccess, LogModification, LogDeletion, LogExport. Each logs to a PhiAuditEntry table. Include the entity and interface. Namespace: GNex.SharedKernel.Compliance.",
            Temperature = 0.1,
            RequestingAgent = Name
        }, ct);
        ...
        {
            Layer = ArtifactLayer.Compliance,
            RelativePath = "GNex.SharedKernel/Compliance/PhiAccessAuditService.cs",
    ...
    private async Task<CodeArtifact> GenerateBreachNotificationPolicy(CancellationToken ct)
    {
        var response = await _llm.GenerateAsync(new LlmPrompt
        {
            ...
            UserPrompt = "Generate a BreachNotificationService with: ReportBreach, AssessRisk (4-factor test), NotifyIndividuals, NotifyHhs, NotifyMedia (>500 records). Include BreachRecord entity. Namespace: GNex.SharedKernel.Compliance.",
            Temperature = 0.1,
            RequestingAgent = Name
        }, ct);
        ...
        {
            Layer = ArtifactLayer.Compliance,
            RelativePath = "GNex.SharedKernel/Compliance/BreachNotificationService.cs",
    ...
    private async Task<CodeArtifact> GenerateMinimumNecessaryPolicy(AgentContext context, CancellationToken ct)
    {
        var response = await _llm.GenerateAsync(new LlmPrompt
        {
            ...
            UserPrompt = "Generate a MinimumNecessaryPolicy static class with method: FilterFields<T>(T entity, string role) that nullifies PHI fields the role should not see. Roles: Physician (all), Nurse (clinical subset), Billing (financial only), Admin (demographics only). Namespace: GNex.SharedKernel.Compliance.",
            Temperature = 0.1,
            RequestingAgent = Name
        }, ct);
        ...
        {
            Layer = ArtifactLayer.Compliance,
            RelativePath = "GNex.SharedKernel/Compliance/MinimumNecessaryPolicy.cs",
    ...
    {
        Layer = ArtifactLayer.Compliance,
        RelativePath = "GNex.SharedKernel/Compliance/PhiClassification.cs",
