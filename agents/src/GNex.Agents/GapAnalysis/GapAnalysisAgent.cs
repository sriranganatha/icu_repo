...
namespace GNex.Agents.GapAnalysis;
...
public sealed class GapAnalysisAgent : IAgent
{
    ...
    {
        // Try to extract from relative path: GNex.PatientService/... → PatientService
        var match = Regex.Match(art.RelativePath, @"GNex\.(\w+Service)", RegexOptions.IgnoreCase);
        if (match.Success) return match.Groups[1].Value;

        // Try from namespace
        match = Regex.Match(art.Namespace, @"GNex\.(\w+Service)", RegexOptions.IgnoreCase);
        if (match.Success) return match.Groups[1].Value;
