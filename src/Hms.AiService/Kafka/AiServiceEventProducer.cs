...
/// <summary>
/// Publishes domain events from AiService to its designated Kafka topic.
/// Uses transactional outbox pattern in production; direct produce in dev.
/// </summary>
public sealed class AiServiceEventProducer : IAsyncDisposable
{
    private readonly IProducer<string, string> _producer;
    private readonly ILogger<AiServiceEventProducer> _logger;
    private readonly string _topic = KafkaTopics.For("ai");

    public AiServiceEventProducer(
        IProducer<string, string> producer,
        ILogger<AiServiceEventProducer> logger)
    {
        _producer = producer;
        _logger = logger;
    }
