...
/// <summary>
/// Publishes domain events from RevenueService to its designated Kafka topic.
/// Uses transactional outbox pattern in production; direct produce in dev.
/// </summary>
public sealed class RevenueServiceEventProducer : IAsyncDisposable
{
    private readonly IProducer<string, string> _producer;
    private readonly ILogger<RevenueServiceEventProducer> _logger;
    private readonly string _topic = KafkaTopics.For("revenue");

    public RevenueServiceEventProducer(
        IProducer<string, string> producer,
        ILogger<RevenueServiceEventProducer> logger)
    {
        _producer = producer;
        _logger = logger;
    }
