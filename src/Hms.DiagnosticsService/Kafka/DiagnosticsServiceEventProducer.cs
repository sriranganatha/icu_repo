...
/// <summary>
/// Publishes domain events from DiagnosticsService to its designated Kafka topic.
/// Uses transactional outbox pattern in production; direct produce in dev.
/// </summary>
public sealed class DiagnosticsServiceEventProducer : IAsyncDisposable
{
    private readonly IProducer<string, string> _producer;
    private readonly ILogger<DiagnosticsServiceEventProducer> _logger;
    private readonly string _topic = KafkaTopics.For("diagnostics");

    public DiagnosticsServiceEventProducer(
        IProducer<string, string> producer,
        ILogger<DiagnosticsServiceEventProducer> logger)
    {
        _producer = producer;
        _logger = logger;
    }
